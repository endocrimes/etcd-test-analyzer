...
BUILDDIR=bin
PKG_NAME=etcd-test-analyzer

GIT_COMMIT := $(shell git rev-parse HEAD)
GIT_DIRTY := $(if $(shell git status --porcelain),+CHANGES)
GO_LDFLAGS := "-X github.com/endocrimes/etcd-test-analyzer/pkg/version.GitCommit=$(GIT_COMMIT)$(GIT_DIRTY)"
...
.PHONY: buildall
buildall: $(foreach GOOSARCH,$(GOOSARCHES),$(BUILDDIR)/$(subst /,,$(dir $(GOOSARCH)))_$(notdir $(GOOSARCH))/$(PKG_NAME)) ## Build for all supported platforms
